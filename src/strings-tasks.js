...
function removeFirstOccurrences(str, value) {
  if (str.includes(value)) {
    ...
    const strFin = str.substring(0, index) + str.substring(index + leng);
    return strFin;
  }
  return str;
}
...
function isPalindrome(str) {
  const newStr = str.replace(/[\W]/g, '').replaceAll(' ', '').toLowerCase();
...
function reverseWords(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    const a = arr[i].split('').reverse().join('');
    finArr.push(a);
  }
  return finArr.join(' ');
}
